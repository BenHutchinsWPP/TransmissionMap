...
// ─── westtec-lines renderer ───────────────────────────────────────────────────

describe('westtec-lines renderer', () => {
  const planned: Record<string, unknown> = {
    name: 'Boardman-Hemingway', dataset: 'planned', scenario: 'Base Case',
    upgrade_type: 'New Line', voltage_kv: 500, line_type: 'AC', length_mi: 62.3,
  };
  const identified: Record<string, unknown> = {
    name: 'Path C Uprate', dataset: 'identified', scenario: 'IDA',
    upgrade_type: 'Uprate', voltage_kv: 345, line_type: null, length_mi: 14.1,
  };

  it('shows project name as title', () => {
    expect(buildPopupHtml('westtec-lines', planned)).toContain('Boardman-Hemingway');
  });

  it('maps scenario code to human label', () => {
    expect(buildPopupHtml('westtec-lines', planned)).toContain('Planned Projects');
    expect(buildPopupHtml('westtec-lines', identified)).toContain('Deliverability (IDA)');
  });

  it('maps dataset to project type label', () => {
    expect(buildPopupHtml('westtec-lines', planned)).toContain('Planned project');
    expect(buildPopupHtml('westtec-lines', identified)).toContain('Identified upgrade');
  });

  it('renders voltage and length with units', () => {
    const out = buildPopupHtml('westtec-lines', planned);
  ...
  it('renders line type when present (planned rows)', () => {
    expect(buildPopupHtml('westtec-lines', planned)).toContain('AC');
  });
});
