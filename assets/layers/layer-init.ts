...
import {
  ...
  DC_LABEL_MINZOOM, DC_LABEL_COLOR, DC_LABEL_HALO,
...
const LAZY_GEOJSON: Record<string, string> = {
  ...
  "eia-ba":                     DATA.eia_ba,
  "us-states":                  DATA.us_states,
  "countries":                  DATA.countries,
  "admin1":                     DATA.admin1_boundaries,
  ...
  "westtec-10yr":               DATA.westtec_10yr,
  "weather-admin-lines":        DATA.admin_lines, // not a registry id — loaded via the weather-live alias below
};
...
const LAYER_SOURCE_ALIAS: Record<string, string> = {
  ...
  // weather-live's own source is an image; enabling it lazily loads the
  // white admin-border lines drawn above the wash instead.
  "weather-live":       "weather-admin-lines",
...
// One raster PMTiles source + layer, id == sourceId. Attribution is passed
// inline (not SOURCE_ATTRIB — see the dedup note in constants.ts).
export function addRasterLayer(id: string, url: string, attribution: string,
                               { opacity = 0.7, resampling = "linear" }: { opacity?: number; resampling?: "linear" | "nearest" } = {}) {
  if (!state.map || state.map.getSource(id)) return;
  state.map.addSource(id, { type: "raster", url: pmtilesUrl(url), tileSize: 256, attribution });
  state.map.addLayer({
    id, type: "raster", source: id,
    layout: { visibility: initialVisibility(id) },
...
export function addTransmissionLines({ sources, sourceLayer, registryId, prefix,
  ...
  for (const t of tiers) {
    ...
    const op = t.id === "hv" && opacity.hvLow !== undefined
      ? ["case", ["<", kvExpr, 200], opacity.hvLow, opacity.hv]
      : opacity[t.id];
    const paint: Record<string, unknown> = { "line-color": color, "line-width": LINE_WIDTH, "line-opacity": op };
    if (undergroundExpr) {
  ...
  // the kV colour still reads between the dashes, but the line visibly ticks.
  // Underground DC ends up dashed-over-dashed; the two periods do not align and
  ...
  if (dcExpr) {
    ...
    addAcross("dc-label", () => ({
      ...
      layout: {
        ...
        "text-field":              "DC",
        "text-font":               ["Noto Sans Regular"],
        "text-size":               ["interpolate", ["linear"], ["zoom"], 6, 10, 10, 12, 14, 14] as unknown as ExpressionSpecification,
        "symbol-spacing":          120,
      ...
      paint: {
        "text-color":      DC_LABEL_COLOR,
        "text-halo-color": DC_LABEL_HALO,
        "text-halo-width": 1.8,
        "text-halo-blur":  0.3,
      },
    }), dcExpr as FilterSpecification);
