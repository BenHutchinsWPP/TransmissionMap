...
import { initMap, switchBasemap, switchProjection } from '../map.js';
import {
...
function resetLayersToDefaults() {
  ...
  if (state.basemap !== "street") switchBasemap("street");
  const streetRadio = document.querySelector<HTMLInputElement>('input[type=radio][name=basemap][value="street"]');
...
function openSourceCredit(sourceId: string) {
  ...
  const target = document.querySelector(`[data-source-credit="${source.creditId}"]`);
