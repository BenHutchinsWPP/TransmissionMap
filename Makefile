...
# GETs off .idx → data/layers/weather_live/{temp.webp,wind.webp,...,meta.json}.
# Needs eccodes + numpy + scipy + pillow in the venv. In production via .github/workflows/weather-data.yml.
weather-live-dev:
	@$(PY) $(SCRIPTS)/fetch_weather_live.py \
		-o data/layers/weather_live
...
# ── Natural Earth admin border lines (weather overlay) ──────────────────────
admin-lines:
	@if [ ! -d "$(VENV)" ]; then echo "ERROR: venv missing. Run 'make install' first."; exit 1; fi
...
# Pure-function tests over the fetch/parse scripts; stdlib unittest, no
# network, no data/ inputs. Same command CI runs (pipeline-tests.yml).
test-pipeline:
