...
- Canadian alerts (ECCC / MSC GeoMet, real polygon geometry) are merged into
  ...
  `country: "US"`). ECCC fetch/parse failure carries forward the previous
  output's CA features (never a hard fail; expired ones are pruned
  client-side) and is recorded via a `feed_status` dict on feature[0]'s
  properties, mirroring the `feed_status` + carry-forward pattern in
  fetch_wildfire_live.py (dict of sub-key → "ok"/"failed", read by
  assets/ui/ui-legends.ts).
...
def load_previous_ca(filepath: str) -> list[dict]:
    """Last-good CA features from the previous output file — carry-forward on
    ECCC failure, mirroring fetch_wildfire_live.py's load_previous_features().
    Expired carried alerts are pruned client-side (nws-staleness.ts)."""
    try:
        ...
        return []
    kept = [ft for ft in prev.get("features", [])
            if (ft.get("properties") or {}).get("country") == "CA"]
    for ft in kept:
        ft["properties"].pop("feed_status", None)
    return kept
...
def main():
    ...
    # ── ECCC (Canada) — carries forward last-good CA alerts on any failure ──
    ...
    try:
        ...
        print(f"WARNING: ECCC fetch/parse failed, carrying forward previous CA alerts: {e}", file=sys.stderr)
        eccc_kept = load_previous_ca(args.output)
        print(f"  carried forward {len(eccc_kept)} previous CA alerts", file=sys.stderr)
    ...
    # Per-subfeed last-successful-pull stamps: fresh on "ok", carried from the
    # previous output on failure — lets the frontend chip show outage duration.
    last_ok_prev = read_prev_feed_last_ok(args.output)
    fc["feed_last_ok"] = {
        feed: generated_utc if status == "ok" else last_ok_prev.get(feed)
