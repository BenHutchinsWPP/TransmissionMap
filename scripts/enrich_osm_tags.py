...
# Geometry type sentinels (passed to jobs list for readability; ignored at runtime)
...
T_VOLTAGE  = re.compile(r'"voltage"=>"(\d+(?:;\d+)*)"')
...
def _to_kv(v: int) -> int:
    """Convert raw OSM voltage (volts or kV) to kV.
    Heuristic: ≥1000 → volts, divide by 1000.  <1000 → already kV."""
    return v // 1000 if v >= 1000 else v


def _best_kv(vs):
    if not vs: return None
    levels = [_to_kv(int(v)) for v in vs.split(';') if v.isdigit()]
...
def _process_layer(shp_path, path_out, _, fdefs, keep_src_fields=None, post_hook=None):  # noqa: _ = wkb_type (unused)
    """Read shp_path, add extra fields from fdefs, write path_out + .csv.

    fdefs: [(name, type_const, width, parser_fn)]
    ...
    # Add derived fields
    for name, ftype, _, parser in fdefs:  # noqa: _ = width (unused)
        nl = name.lower()
        if has_direct_kv:
            dc = col_lower.get(nl)
                ...
                if vc:
                    def _derive_kv(v):
                        ...
                        if vs and vs != 'None':
                            ...
                            return max(nums) // 1000 if nums else -1
                        return -1
                    gdf[name] = gdf[vc].apply(_derive_kv).astype('int32')
                    continue
...
# ---------------------------------------------------------------------------
line_fdefs = [
    ("nominal_kv",  OFTInteger, 6,   lambda o: _best_kv(_get(o, T_VOLTAGE)) or -1),
    ("cables",      OFTInteger, 6,   lambda o: _get_int(o, T_CABLES) or -1),
    ...
    ("wires",       OFTString,  20,  lambda o: _get(o, T_WIRES) or ''),
    ("ref",         OFTString,  40,  lambda o: _get(o, T_REF) or ''),
    ("const",       OFTString,  40,  lambda o: _get(o, T_CONSTR) or ''),
]

# Pipeline facility areas (pipeline=substation, pipeline=aqueduct, pipeline=siphon, etc.)
pipeline_line_fdefs = [
    ("pipeline",    OFTString, 40, lambda o: _get(o, T_PIPELINE) or ''),
    ("facil_type",  OFTString, 40, lambda o: _get(o, T_SUBTYPE) or ''),
    ("substance",   OFTString, 40, lambda o: _get(o, T_SUBSTANCE) or ''),
    ("operator",    OFTString, 80, lambda o: _get(o, T_OP) or ''),
    ("op_wikidata", OFTString, 20, lambda o: _get(o, T_OP_WD) or ''),
]
...
def main():
    ...
    jobs = [
        ...
        ("Pipeline routes",   "pipeline_lines.shp",            wkbLineString, pipeline_line_fdefs, PIPELINE_LINE_FIELDS, _drop_nonfuel),
        ("Pipeline features", "pipeline_feature_lines.shp",    wkbLineString, pipeline_line_fdefs, PIPELINE_LINE_FIELDS, None),
        ("Pipeline feat pts", "pipeline_feature_points.shp",   wkbPoint,      pipeline_pt_fdefs,   PIPELINE_PT_FIELDS,   None),
    ]

    for label, pat, wkb_t, fdefs, keep_src, post_hook in jobs:
        ...
        for shp in files:
            ...
            try:
                _process_layer(shp, out, wkb_t, fdefs, keep_src, post_hook=post_hook)
