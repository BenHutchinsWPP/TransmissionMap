...
from osm_common import SHARED_FILTER_TAGS, _run
    ...
    for pbf_path in pbf_files:
        ...
        if stem.endswith("_filtered"):
            ...
            if (filtered.exists()
                ...
                if not _run(cmd, "osmium tags-filter", check=False):
        ...
        if not all_na and bb and bb.get("min_lon") is not None:
            ...
            if _run(cmd, "osmium extract", check=False):
        ...
        for feat in polygon_features:
            ...
            if not _run(cmd, f"osmium export {feat}", check=False):
        ...
        for feat in enabled_features:
            ...
            for key, vals in rules.items():
                ...
                for layer in layers:
                    ...
                    _run(cmd, f"ogr2ogr {safe}/{layer}", check=False)
