    ...
    for text in csv_texts:
        for row in csv.DictReader(io.StringIO(text)):
            if (row.get("confidence") or "").lower() == "l":
                continue
            # Skip rows with missing/junk coordinates instead of crashing the feed.
            try:
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=5) as pool:
        ...
        # unavailable source doesn't blank the whole hourly update.
    ...
    # Per-subfeed last-successful-pull stamps: fresh on "ok" (and smoke's
    # "fallback-Nd", which is a successful pull of an older product day),
    # carried from the previous output on degraded/failed — lets the frontend
    # chip show how old the carried data is.
    last_ok_prev = read_prev_feed_last_ok(args.output)
    fc["feed_last_ok"] = {
        feed: generated_utc if status == "ok" or status.startswith("fallback-")
        else last_ok_prev.get(feed)
