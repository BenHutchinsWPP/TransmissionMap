...
> & SPR** (`pol_terminal` + `spr` — ~2,265). The petroleum layer's registry entry
> is currently commented out (`src/registry/pipelines.ts` — no panel row or URL
> code; uncomment to restore). Gas Facilities sits in the **Fuel Delivery** panel
> group; the "Facility type" legend filter is built to span both.
