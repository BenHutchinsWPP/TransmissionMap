...
Four **live** feature types refreshed every 30 min, sharing one GeoJSON file. Three registry
layers consume it:
...
The FeatureCollection top level mirrors `generated_utc`/`feed_status` and adds
`feed_last_ok` (subfeed → ISO stamp of its last successful pull, carried from the
previous output while a subfeed is down) — the chips use it to show outage duration
("CA perim down 3h").
...
- **Live & best-effort.** Upstream feeds go down; failure handling differs by feed:
  ...
  - **Perimeters / incidents / smoke (secondary):** each degrades independently to
    ...
    before resorting to the previous run) so one unavailable source doesn't block
