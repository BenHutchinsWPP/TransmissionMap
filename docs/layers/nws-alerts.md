...
| **Refresh** | `.github/workflows/alerts-outages.yml` (shared with the ODIN outage feed; the two fetch scripts run in parallel, failure-isolated) — primary trigger `workflow_dispatch`, fired every 10 min by one cron-job.org job, plus an insurance `schedule:` cron every **2 h** (`2 */2`) so the feed can't sit stale past the 3 h kill-switch if cron-job.org dies. Joins the shared `data-branch` concurrency group (also used by `wildfire-data.yml` and `weather-data.yml`) so force-pushes to the orphan `data` branch don't collide. Failed runs surface only as a stale/aging snapshot on the map (no automatic issue). |
...
FeatureCollection itself now always carries four top-level keys, written even when
`features` is empty:
...
| `feed_last_ok` | `{"eccc": <ISO stamp|null>}` — the subfeed's last *successful* pull: `generated_utc` when ok, carried from the previous output while down (`geo_common.read_prev_feed_last_ok()`). The legend chip uses it to show outage duration ("Canada down 3h") |
...
`ends`, but the popup lists every alert on that zone/county. The winner is recomputed
against the active legend-chip filter: with the "fire" chip selected, a county holding a
Flood Warning (winner) plus a Red Flag Warning recolors to the fire color and stays
visible — a filtered group never hides a zone/county that has a matching alert. Polygons
carrying more than one (filter-matching) alert render a thicker outline as a
multiple-alerts cue.
...
filters can't see feature-state, `setZoneGroupFilter()` re-runs the feature-state join
with the filter applied — winner picked among matching alerts only, non-matching
features unpainted — instead of `setFilter` (wired from `assets/filters.ts`'s
`applyNwsGroupFilter`). Clicking a joined zone/county lists **every**
...
An ECCC fetch/parse failure **carries forward the previous output's CA features**
(`load_previous_ca()` — expired ones self-prune client-side via `nws-staleness.ts`),
never a hard failure of the whole pull: `feed_status: {"eccc": "failed"}` is set on
`features[0]`'s properties (`"ok"` on success), read by `assets/ui/ui-legends.ts` for
the amber degraded-feed chip — the same carry-forward + `feed_status` pattern
`fetch_wildfire_live.py` uses for its live sub-feeds.
