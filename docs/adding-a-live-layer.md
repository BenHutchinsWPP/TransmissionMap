...
emits **several files** (e.g. a raster: image + `.i16.gz` LUT + `.json` meta)
...
pipeline layers live on the `data-static` branch behind `DATA_ORIGIN`. There is no shared constant for the live origin — each
live entry spells out the DEV/PROD ternary, so local dev reads a file you
generated by running the fetch script by hand:
...
readout rides on a **sidecar LUT**: a flat, gzipped `.i16.gz` grid of scaled integers plus
...
- [ ] `DATA` entry in `assets/constants.ts` (live origin, not `DATA_ORIGIN`)
...
- [ ] Raster only: `.i16.gz` LUT + `.json` meta emitted, wired into `RASTER_PROBES`
