...
// ─── HVDC detection ──────────────────────────────────────────────────────────
// Colour is spoken for (kV) and dashes are spoken for (overhead/underground),
// so DC circuits get two marks that use neither:
//   1. white dashes over the base stroke — the far-out cue, visible from z0
//   2. a repeating "DC" label along the route from z6 — the unambiguous cue
// The stripe carries its own width ramp rather than a fraction of LINE_WIDTH:
// at z3-5 the base line is ~1px, so a 0.4x stripe rounded to a sub-pixel ghost
// and DC was indistinguishable when zoomed out.  This floors it at 0.9px.
export const OSM_DC_EXPR = ["==", ["coalesce", ["get", "is_dc"], 0], 1] as ExpressionSpecification;
export const HIFLD_DC_EXPR = ["==", ["index-of", "DC", ["coalesce", ["get", "TYPE"], ""]], 0] as ExpressionSpecification;
export const DC_STRIPE_COLOR = "#f8fafc";
export const DC_STRIPE_WIDTH = ["interpolate", ["linear"], ["zoom"],
  3, 0.9,
  7, 1.1,
  10, 1.5,
  13, 2.0
] as unknown as ExpressionSpecification;
// Dash lengths are multiples of line-width, so this stays ~even on/off at any zoom.
export const DC_STRIPE_DASH = [2, 1.8];
export const DC_LABEL_MINZOOM = 6;
export const DC_LABEL_COLOR = "#0f172a";
export const DC_LABEL_HALO = "#f8fafc";
...
export const KV_BUCKETS = [
  ...
  { id: "100-199", urlCode: "P", label: "100–199", color: "#c587a6" },
  { id: "<100",    urlCode: "L", label: "< 100",   color: "#bdab75" },
  { id: "unknown", urlCode: "U", label: "Unknown", color: "#a39dbe" },
];

// Same buckets with display labels ("500–549 kV") for the legend.
export const VOLTAGE_LEGEND = KV_BUCKETS.map(b =>
  b.id === "unknown" ? b : { ...b, label: `${b.label} kV` });
...
export function bucketMatchExpr(
  ...
  // MapLibre "match" requires a trailing fallback; buckets without an "other"
  // (WestTEC — every row has a real value) fall back to neutral gray, which
  // is never hit in practice.
  expr.push(buckets.find(b => b.id === "other")?.color ?? "#94a3b8");
  return expr as unknown as ExpressionSpecification;
}
...
// ─── WestTEC 10-Year Plan color-by ───────────────────────────────────────────
// Drives westtec-10yr line color; selected via state.westtecColorBy. Every
// feature has a real value for both fields — no "other"/catch-all bucket.
export const WESTTEC_SCENARIO_BUCKETS = [
  { id: "base_case",  urlCode: "B", label: "Planned Projects",        color: "#1e40af" },
  { id: "sra",        urlCode: "R", label: "Reliability-Driven (SRA)", color: "#4ade80" },
  { id: "ida",        urlCode: "D", label: "Deliverability (IDA)",     color: "#38bdf8" },
  { id: "congestion", urlCode: "C", label: "Congestion",               color: "#dc2626" },
];
export const WESTTEC_SCENARIO_MAP = {
  base_case:  ["Base Case"],
  sra:        ["SRA"],
  ida:        ["IDA"],
  congestion: ["Congestion"],
...
export const WESTTEC_DATASET_BUCKETS = [
  { id: "planned",    urlCode: "P", label: "Planned Projects",    color: "#2563eb" },
  { id: "identified", urlCode: "I", label: "Identified Upgrades", color: "#16a34a" },
];
export const WESTTEC_DATASET_MAP = {
  planned:    ["planned"],
  identified: ["identified"],
};

export type WesttecColorMode = "scenario" | "dataset";
const WESTTEC_COLOR_CFG: Record<WesttecColorMode, {
  field: string;
  buckets: { id: string; color: string }[];
  map: Record<string, string[]>;
}> = {
  scenario: { field: "scenario", buckets: WESTTEC_SCENARIO_BUCKETS, map: WESTTEC_SCENARIO_MAP },
  dataset:  { field: "dataset",  buckets: WESTTEC_DATASET_BUCKETS,  map: WESTTEC_DATASET_MAP },
};

export function westtecColorExpr(mode: WesttecColorMode): ExpressionSpecification {
